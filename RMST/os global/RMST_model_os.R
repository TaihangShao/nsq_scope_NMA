...
setwd("C:/Users/ECHO/Desktop/nsq-cea-scope/RMST/os global")
path<-"C:/Users/ECHO/Desktop/nsq-cea-scope/RMST/os global"
data <- read.csv("RP_OS_globalsub.csv")
...
# Three arm trial with 5 as the reference category
id3 <- 5

for(i in id3) {
  for(j in c(4,6)) {
    temp1 <- data[data$studyCode==i,]
    temp1 <- temp1[temp1$txCode==5 | temp1$txCode==j,]
...
studies <- data.frame(studyCode=c(1,2,3,4,6,5,5,5),
                      study=c("Checkmate227", "CheckMate9LA", "Impower130", "Impower132",
                              "Keynote189","Impower150","Impower150","Impower150"),
                      t1=c(1,1,1,1,1,5,5,4),
                      t2=c(2,3,4,4,7,4,6,6))
...
write.csv(rmst_18, "rmst_18_os_gs.csv", row.names=F)
...
rmst_18 <- read.csv("rmst_18_os_gs.csv")
...
write.csv(fe_results,file="fe_results_os_gs.csv")
...
data <- read.csv("fe_results_os_gs.csv") 
data <- data[1:6,]

data$trt <- c("ipi+niv", "niv+ipi+che", "ate+che", "bev+che", "ate+bev+che", "pem+che")
...
# Variable for rank
data$rank_code <- c(rep(1, 7), rep(2, 7), rep(3, 7), rep(4, 7), rep(5, 7),
                    rep(6, 7), rep(7, 7))
...
data$Treatment <- rep(c("che","ipi+niv", "niv+ipi+che", "ate+che", "bev+che", "ate+bev+che", "pem+che"),7)
...
q <- ggplot(data, aes(x=rank_code, y=Treatment)) +
  ...
  scale_x_continuous(name="Rank", limits=c(1, 7), breaks=seq(1,7,1)) +
  scale_y_discrete(name="Treatment") +  
  geom_text(aes(label=prob))
q

# save plot
dev.copy(png, "rank_plot_os_gs.png")
dev.off()

write.csv(forest_data,"forest_data_os_gs.csv",row.names =FALSE)
