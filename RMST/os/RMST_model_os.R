...
setwd("C:/Users/ECHO/Desktop/nsq-cea-scope/RMST/os")
...
studies <- data.frame(studyCode=c(1,2,3,4,5,7,8,9,6,6,6),
                      study=c("CameL", "Checkmate227", "CheckMate9LA", "Impower130", "Impower132",
                              "Keynote189", "Orient11", "Tasuki52", "Impower150","Impower150","Impower150"),
                      t1=c(1,1,1,1,1,1,1,6,6,6,5),
                      t2=c(2,3,4,5,5,8,9,10,5,7,7))
...
write.csv(rmst_18, "rmst_18_os.csv", row.names=F)
...
rmst_18 <- read.csv("rmst_18_os.csv")
...
write.csv(fe_results,file="fe_results_os.csv")
...
data <- read.csv("fe_results_os.csv") 
data <- data[1:9,]

data$trt <- c("cam+che", "ipi+niv", "niv+ipi+che", "ate+che", "bev+che", "ate+bev+che", "pem+che", "sin+che", "niv+bev+che")
...
# Variable for rank
data$rank_code <- c(rep(1, 10), rep(2, 10), rep(3, 10), rep(4, 10), rep(5, 10),
...
data$Treatment <- rep(c("che","cam+che", "ipi+niv", "niv+ipi+che", "ate+che", "bev+che", "ate+bev+che", "pem+che", "sin+che", "niv+bev+che"),10)
...
q <- ggplot(data, aes(x=rank_code, y=Treatment)) +
  ...
  scale_x_continuous(name="Rank", limits=c(1, 10), breaks=seq(1,10,1)) +
  scale_y_discrete(name="Treatment") +  
  geom_text(aes(label=prob))
q

# save plot
dev.copy(png, "rank_plot_os.png")
dev.off()

write.csv(forest_data,"forest_data_os.csv",row.names =FALSE)
