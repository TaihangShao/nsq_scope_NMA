...
path<-"C:/Users/ECHO/Desktop/nsq-cea-scope/RMST/pfs tpss1"
setwd(path)
...
studies <- data.frame(studyCode=c(1,2,3,4,5),
...
write.csv(rmst_18, "rmst_18_pfs_tpss1.csv", row.names=F)
...
rmst_18 <- read.csv("rmst_18_pfs_tpss1.csv")
...
write.csv(fe_results,file="fe_results_pfs_tpss1.csv")
...
data <- read.csv("fe_results_pfs_tpss1.csv") 
data <- data[1:5,]

data$trt <- c("niv+ipi+che", "ate+che",  "pem+che", "sin+che", "tis+che")
...
data$Treatment <- rep(c("che","niv+ipi+che", "ate+che", "pem+che", "sin+che", "tis+che"),6)
...
q <- ggplot(data, aes(x=rank_code, y=Treatment)) +
  ...
  labs(title = "Rank plot for PFS with PD_L1 expression ≤ 1%")+
...
# save plot
dev.copy(png, "rank_plot_pfs_tpss1.png")
dev.off()

write.csv(forest_data,"forest_data_pfs_tpss1.csv",row.names =FALSE)
