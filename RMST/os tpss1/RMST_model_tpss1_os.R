...
path<-"C:/Users/ECHO/Desktop/nsq-cea-scope/RMST/os tpss1"
setwd(path)
...
studies <- data.frame(studyCode=c(1,2,4,3,3,3),
                      study=c("Checkmate9LA","Impower132",
...
write.csv(rmst_18, "rmst_18_os_tpss1.csv", row.names=F)
...
rmst_18 <- read.csv("rmst_18_os_tpss1.csv")
...
write.csv(fe_results,file="fe_results_os_tpss1.csv")
...
data <- read.csv("fe_results_os_tpss1.csv") 
data <- data[1:5,]
...
q <- ggplot(data, aes(x=rank_code, y=Treatment)) +
  ...
  labs(title = "Rank plot for OS with PD_L1 expression ≤ 1%")+
...
# save plot
dev.copy(png, "rank_plot_os_tpss1.png")
dev.off()

write.csv(forest_data,"forest_data_os_tpss1.csv",row.names =FALSE)
