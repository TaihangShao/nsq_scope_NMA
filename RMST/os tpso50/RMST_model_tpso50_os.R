...
path<-"C:/Users/ECHO/Desktop/nsq-cea-scope/RMST/os tpso50"
setwd(path)
...
studies <- data.frame(studyCode=c(1,2,4,3,3,3),
                      study=c("Checkmate227","Impower132",
...
write.csv(rmst_18, "rmst_18_os_tpso50.csv", row.names=F)
...
rmst_18 <- read.csv("rmst_18_os_tpso50.csv")
...
write.csv(fe_results,file="fe_results_os_tpso50.csv")
...
data <- read.csv("fe_results_os_tpso50.csv") 
data <- data[1:5,]

data$trt <- c("ipi+niv", "ate+che", "bev+che", "ate+bev+che", "pem+che")
...
data$Treatment <- rep(c("che","ipi+niv", "ate+che", "bev+che", "ate+bev+che", "pem+che"),6)
...
q <- ggplot(data, aes(x=rank_code, y=Treatment)) +
  ...
  labs(title = "Rank plot for OS with PD_L1 expression ≥ 50%")+
...
# save plot
dev.copy(png, "rank_plot_os_tpso50.png")
dev.off()

write.csv(forest_data,"forest_data_os_tpso50.csv",row.names =FALSE)
