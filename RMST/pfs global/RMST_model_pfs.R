...
path<-"C:/Users/ECHO/Desktop/nsq-cea-scope/RMST/pfs global"
setwd("C:/Users/ECHO/Desktop/nsq-cea-scope/RMST/pfs global")
data <- read.csv("RP_PFS_globalsub.csv")
...
# Three arm trial with 4 as the reference category
id3 <- 4

for(i in id3) {
  for(j in c(3,5)) {
    temp1 <- data[data$studyCode==i,]
    temp1 <- temp1[temp1$txCode==4 | temp1$txCode==j,]
...
# Look at 3 v 5 in three arm trial

temp1 <- data[data$studyCode==4,]

# Re-code treatments as 0 and 1
temp1$newTx[temp1$txCode==3] <- 0
temp1$newTx[temp1$txCode!=3] <- 1
...
studies <- data.frame(studyCode=c(1,2,3,5,4,4,4),
                      study=c( "CheckMate9LA", "Impower130", "Impower132",
...
write.csv(rmst_18, "rmst_12_pfs_gs.csv", row.names=F)
...
rmst_18 <- read.csv("rmst_12_pfs_gs.csv")
...
write.csv(fe_results,file="fe_results_pfs_gs.csv")
...
data <- read.csv("fe_results_pfs_gs.csv") 
data <- data[1:5,]

data$trt <- c("niv+ipi+che","ate+che", "bev+che", "ate+bev+che", "pem+che")
...
# Variable for rank
data$rank_code <- c(rep(1, 6), rep(2, 6), rep(3, 6), rep(4, 6), rep(5, 6),
                    rep(6, 6))
...
data$Treatment <- rep(c("che", "niv+ipi+che","ate+che", "bev+che", "ate+bev+che", "pem+che"),6)
...
q <- ggplot(data, aes(x=rank_code, y=Treatment)) +
  ...
  scale_x_continuous(name="Rank", limits=c(1, 6), breaks=seq(1,6,1)) +
  scale_y_discrete(name="Treatment") +  
  geom_text(aes(label=prob))
q

# save plot
dev.copy(png, "rank_plot_pfs_gs.png")
dev.off()

write.csv(forest_data,"forest_data_pfs_gs.csv",row.names =FALSE)
